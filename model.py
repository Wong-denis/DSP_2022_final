...
class Model:
    ...
    def _predict(self, holdout_type):
        ...
        elif holdout_type == "classify":
            X_holdout, y_holdout = self.X,self.y
            scaler = self.best_estimator['scaler']
            model = self.best_estimator['model']
            X_holdout_scaled = scaler.transform(X_holdout)
            y_pred = model.predict(X_holdout_scaled)
            print('y_pred',y_pred)
            return list(self.encoder.inverse_transform(list(y_pred)))
        ...
        X_holdout_scaled = scaler.transform(X_holdout)
        y_pred = model.predict(X_holdout_scaled)
        #print('X_holdup_scaled',X_holdout_scaled)
        #print('y_pred',y_pred)
    ...
    def predict(self, holdout_type):
        ...
        print(f'{holdout_type} Precision per Class: {TP / (TP + FP)}')
        print(f'{holdout_type} Sensitivity per Class: {TP / (TP + FN)}')
